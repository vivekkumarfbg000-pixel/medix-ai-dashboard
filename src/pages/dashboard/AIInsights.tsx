...
import { drugService, DrugInfo, InteractionResult } from "@/services/drugService";
...
const AIInsights = () => {
  ...
  const handleDrugSearch = async () => {
    ...
    try {
      const info = await drugService.searchDrug(drugQuery);
      if (info) {
        setDrugInfo(info);
      } else {
        toast.error("Drug information not found. Try a different name.");
      }
    } catch (error) {
  ...
  const checkInteractions = async () => {
    ...
    try {
      ...
      if (results.length === 0) {
        toast.success("No critical interactions found (Simulation).");
      }
    } catch (error) {
        ...
        <TabsContent value="drug-info" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Search for drug details, side effects, and benefits from OpenFDA
              </CardDescription>
            </CardHeader>
                  ...
                  <ul className="space-y-2">
                    {drugInfo.sideEffects.map((effect: any, i: number) => (
                      <li key={i} className="flex items-center justify-between text-sm">
                        <span className="line-clamp-2">{effect.effect}</span>
                        <Badge
                          variant={
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      <AlertTriangle className={`w-5 h-5 flex-shrink-0 ${interaction.severity === "severe" ? "text-destructive" : "text-warning"
                        }`} />
      ...
      <Card className="bg-muted/30">
        <CardContent className="p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Brain className="w-5 h-5 text-primary" />
            <div>
              <p className="text-sm font-medium">Powered by OpenFDA</p>
              <p className="text-xs text-muted-foreground">
                Data sourced from open.fda.gov and National Library of Medicine
              </p>
            </div>
          </div>
